...
void FdtManager::early_scan_mem(uint64 fdt_paddr, uint64 *base, uint64 *size) {
  ...
  while (true) {
    uint32 token =
        bswap32(*reinterpret_cast<const uint32 *>(dt_struct + offset));
    offset += 4;
    ...
    if (token == FDT_BEGIN_NODE) {
      ...
      uint32 len =
          bswap32(*reinterpret_cast<const uint32 *>(dt_struct + offset));
      uint32 nameoff =
          bswap32(*reinterpret_cast<const uint32 *>(dt_struct + offset + 4));
      offset += 8;
      ...
      if (depth >= 0 && depth < 16) {
        ...
        if (str_equals(prop_name, "#address-cells")) {
          address_cells[depth] =
              bswap32(*reinterpret_cast<const uint32 *>(prop_val));
        ...
        // Decode the "reg" property using the parent's cell sizes
          ...
          *size = read_cells(prop_val + address_cells[p_depth] * 4,
                             size_cells[p_depth]);
          return; // Success
        }
      }
    ...
    if (token == FDT_BEGIN_NODE) {
      ...
      if (depth >= 0 && depth < 16) {
        if (str_equals(prop_name, "#address-cells")) {
          ...
          if (prop_contains_string(reinterpret_cast<const char *>(prop_val),
                                   len, "ns16550a")) {
            in_uart_node = true;
          ...
          current_node_size = read_cells(prop_val + address_cells[p_depth] * 4,
                                         size_cells[p_depth]);
...
DeviceNode *FdtManager::get_node_by_path(const char *path) {
  ...
  while (*path && cur) {
    ...
    while (lc != le) {
      DeviceNode *child =
          list_entry<DeviceNode, &DeviceNode::sibling_link_>(lc);
...
void fdt_unflatten() { g_fdt->unflatten(); }
...
void FdtManager::early_scan_kaslr(uint64 fdt_paddr, uint64 *seed)
{
    if (!seed) return;
    *seed = 0;

    const fdt_header *hdr = reinterpret_cast<const fdt_header *>(fdt_paddr);
    if (bswap32(hdr->magic) != FDT_MAGIC) return;

    const uint8 *dt_struct = reinterpret_cast<const uint8 *>(fdt_paddr) + bswap32(hdr->off_dt_struct);
    const char *dt_strings = reinterpret_cast<const char *>(fdt_paddr) + bswap32(hdr->off_dt_strings);
    ...
    while (true) {
        uint32 token = bswap32(*reinterpret_cast<const uint32 *>(dt_struct + offset));
        offset += 4;
        ...
        if (token == FDT_BEGIN_NODE) {
            ...
            depth++;
            if (depth == 1 && str_starts_with(name, "chosen")) {
                in_chosen = true;
            } else if (depth == 1) {
                in_chosen = false;
            }
            ...
            uint32 len = bswap32(*reinterpret_cast<const uint32 *>(dt_struct + offset));
            uint32 nameoff = bswap32(*reinterpret_cast<const uint32 *>(dt_struct + offset + 4));
            offset += 8;

            if (in_chosen) {
                const char *prop_name = dt_strings + nameoff;
                if (str_starts_with(prop_name, "kaslr-seed") && len == 8) {
                    *seed = read_cells(dt_struct + offset, 2);
                    return;
                }
            }
            offset = (offset + len + 3) & ~3u;
...
void fdt_early_get_kaslr_seed(uint64 *seed) {
  FdtManager::early_scan_kaslr(g_fdt->get_paddr(), seed);
}
